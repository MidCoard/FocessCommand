...
public class CommandRoute {
    ...
    public CommandRoute(@NotNull CommandSender sender, @NotNull String input, @NotNull List<Token> tokens) {
        this.sender = sender;
        this.input = input;
        this.tokens = tokens;
    }
    ...
    /**
     * Resolves the route by searching through the registered commands in the manager.
     */
    @NotNull
    public CommandRoute resolve(@NotNull CommandManager manager) {
        if (tokens.isEmpty()) {
            this.state = CommandResult.NONE;
            return this;
        }
            ...
            if (sender.hasPermission(executor.getPermission()) && executor.getExecutorPermission().test(sender)) {
                ...
                if (this.matchedExecutor == null) {
                    ...
                    if (data != null) {
                        ...
                        this.state = CommandResult.ALLOW;
                    }
                }
            }
        }
        ...
        if (this.matchedExecutor == null) {
            this.state = anyVisible ? CommandResult.ARGS_NOT_EXECUTED : CommandResult.NONE;
        }
    ...
    public ExecutionResult execute() {
        ...
        if (this.command == null || this.state == CommandResult.NONE)
            return ExecutionResult.of(CommandResult.NONE);
        
        if (this.matchedExecutor == null) {
            if (this.state != CommandResult.COMMAND_REFUSED) {
                this.command.infoUsage(this.sender);
                return ExecutionResult.of(this.state == CommandResult.ARGS ? CommandResult.ARGS : CommandResult.ARGS_NOT_EXECUTED);
            }
            return ExecutionResult.of(CommandResult.COMMAND_REFUSED);
        }

        try {
            CommandResult result = this.matchedExecutor.getExecutor().execute(this.sender, this.matchedData);
            
            // Handle CommandResultExecutors
    ...
    @NotNull
    public List<String> getTokens() { return tokens.stream().map(t -> t.content).collect(Collectors.toList()); }
    @Nullable
